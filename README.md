...
- **Project instructions, skills, tools and MCP**: each project has one versioned shared-instructions document, progressively selected skills, project-scoped Streamable HTTP MCP connections, and a bounded tool-calling loop. Every routed model receives the same active instructions; external agents receive them too. Models can search/list/read project context and delegate a bounded task to an existing specialist agent. MCP credentials stay server-side and API responses expose only header names. Configure these from **Instructions & Tools** in the project sidebar.
