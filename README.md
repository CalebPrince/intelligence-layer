...
**Last updated:** 2026-09-19 (Inteli-Space now has live GitHub OAuth, repository import, full text/code context ingestion, automatic large-file hydration when a chat names a file, signed push/deployment webhooks, persistent Railway storage, and a bidirectional shared-memory bridge for prince-web-app agents; agent GitHub write proposals now have Auto/Manual/Reject modes in the backend.)
...
- **The other 16 prince-web-app agents — genuinely bidirectional shared memory** (built after Sage, going further than the original "one agent at a time" plan): Wendy, Chief, Chloe, Lisa, Content, Beacon, Dossier, Nurturer, Proposal, Arch, Ada, Sketch, Scout, Radar, Reel and Allie all chat through `POST /v1/agents/admin/chat` (`app/agent_client.py`), authenticated to prince-web-app's real `POST /api/v1/admin/agents/{key}/chat` via a **narrow service credential**, not the human admin's session: prince-web-app's `AuthMiddleware::requireAgentAuth()` accepts a separate `MODEL_AGNOSTIC_AGENT_TOKEN` (timing-safe compared, `hash_equals`) but *only* on the `/api/v1/admin/agents/*/chat` route pattern — it can't be used for anything else prince-web-app's admin API can do. Memory is a real bridge, not a copy in each app: `app/database.py`'s `get_shared_agent_memory`/`append_shared_agent_memory` store the transcript here, exposed to prince-web-app over `GET/POST /v1/agents/shared-memory/{agent_key}` (gated by `X-Shared-Memory-Token` / `SHARED_MEMORY_TOKEN`), and prince-web-app's own `src/Support/SharedAgentMemory.php` calls that bridge from inside `AiAgentEngine.php` — the same code path every admin chat in prince-web-app's own UI runs through — so a conversation started in Inteli-Space and continued natively in prince-web-app (or vice versa) is the same conversation. Requires `PRINCE_WEB_APP_ADMIN_TOKEN` and `SHARED_MEMORY_TOKEN` configured on this backend, and the matching `MODEL_AGNOSTIC_AGENT_TOKEN`/`MODEL_AGNOSTIC_MEMORY_TOKEN` on prince-web-app — not yet set in local `.env` (check before assuming this works on a fresh checkout). Each reply is saved as an ordinary model response and can be accepted as a decision, same as Sage.
...
- **Agents (`/agents`)**: "Your AI Team" hero, stat tiles, status tabs, tags and task counts, Create New Agent / Agent Templates tiles, an Agent Activity feed and Quick Actions. **17 of the roster's cards are real agents, not lookalikes** — Sage via the public route (`live: true`, "Live agent" badge), and Wendy, Chief, Chloe, Lisa, Content, Beacon, Dossier, Nurturer, Proposal, Arch, Ada, Sketch, Scout, Radar, Reel and Allie via the authenticated admin adapter (`chatKey`, plain "Chat with {name}" button) — see "What's working" above for how the memory is genuinely shared with prince-web-app either way, and note the admin ones need `PRINCE_WEB_APP_ADMIN_TOKEN`/`SHARED_MEMORY_TOKEN` configured to actually reach prince-web-app. Every chat opens a real modal (`AgentChatModal`/the Sage-specific one), restores history via `GET /v1/agents/{key}/history`, and a reply can be **Accept as decision**'d like any model response. Leo, Maya and Kai remain illustrative placeholders (no corresponding prince-web-app agent) — there is no backend concept of an agent "task" or "status" for them.
...
- [ ] **Finish GitHub write proposals in Chat.** The backend now persists `auto` / `manual` / `reject` modes and can commit approved files to a new branch, but Chat still needs the proposal parser, file diff preview, approval buttons, and a link to the resulting branch/commit.
...
- [ ] **Agents — real actions beyond chat, for the agents that have one.** Chat (with real shared memory) now works for all 16 non-Sage agents once the tokens above are set. What's still chat-only: Ledger drafting/sending a real proposal, Ada drafting/sending a real invoice, Jason (Nurturer) sending real outreach, etc. — see `ProposalController::send`, `InvoiceController::send`, `NurturerController::adminSendReply` in prince-web-app. These have real side effects on a live production app (real client leads/invoices/proposals) — verify one end-to-end (in a way that's easy to undo) before wiring more.
