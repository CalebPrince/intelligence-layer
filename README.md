...
└── frontend/             Next.js App Router + TypeScript + Tailwind
    ...
    └── src/
        ...
        │                      activity, projects, integrations, agents, settings and help
...
- **Chat (`/chat`)**, rebuilt to match its reference: the first model response uses the Project Intelligence comparison treatment, while later turns use normal left-aligned model bubbles and right-aligned user bubbles. Chat text controls the default collaboration: no provider name sends the question to all active models, one name such as "Hi Claude" sends it only to Claude, and two or more names such as "Claude and ChatGPT" send only those models into the deliberation flow so they can inspect each other's answers before synthesis. The Auto / ChatGPT / Claude / Gemini / Parallel / Deliberate pills remain available for explicit routing, but provider mentions take precedence. The chat grounding prompt includes the current project brief and the ranked Project Context Library, now up to 40,000 characters per request. Right panel "Project Context": stats, Active Context / Relevant Files (ranked by overlap with your last question) / Recent Activity tabs, **Sync now** (re-reads the project's folder), "Used for this response" (the context items actually sent, marked "shortened" if trimmed) and Recent Decisions. The header has Share and `...`; the sidebar Chat item shows the conversation count. **Clear chat** deletes the latest native conversation through `DELETE /v1/projects/{id}/chat`, so it stays cleared after reload.
...
- **Models go live with their keys**: a model is active as soon as its provider's key is set in the deployed environment (the `is_active` flags in `config.py` are only the fallback). Production provider keys are already configured, and live `/v1/models` verification confirms Claude, ChatGPT and Gemini are active.
- **Project instructions, skills, tools and MCP**: each project has one versioned shared-instructions document, progressively selected skills, project-scoped Streamable HTTP MCP connections, and a bounded tool-calling loop. Every routed model receives the same active instructions; external agents receive them too. From Chat, say “I want to create a skill for designing buttons for the website” and the model drafts a proposal in the conversation; choose **Add to skills** or **Reject** before anything is persisted. Invoke an approved skill later with `/button-design` at the start of a message. Exact slash commands load that skill instead of relying only on description matching. Models can search/list/read project context and delegate a bounded task to an existing specialist agent. MCP credentials stay server-side and API responses expose only header names. Configure these from **Instructions & Tools** in the project sidebar.
...
- **Chat conversation list**: `GET /v1/projects/{id}/conversations` (title, turn count, last message preview and time, newest first); `GET .../chat/history` takes an optional `?conversation_id=`; New chat creates a persisted conversation and Recent chats in the Recent Activity tab reopen the full selected thread. Clear chat deletes the selected native conversation.
...
- **Shells**: project pages use the project sidebar (switcher, models, agents). Workspace pages (`/dashboard`, `/projects`, `/integrations`, `/agents`, `/analytics`, `/settings`, `/help`) use the workspace sidebar (Main Dashboard, All Projects, Chat, Decisions, Context Library, Integrations, Agents, Analytics, Settings, models with a "More Models" row, "Pro Plan" footer as in the reference) and a top bar with **+ New Project**. Help includes searchable guides, FAQs, shortcuts and troubleshooting links.
...
- **Settings (`/settings`)**: complete workspace configuration for General, Account, Appearance, Models, Global Instructions, MCP Connections, Notifications, Security, Team and Billing. General settings use the typed `settings` table; the remaining editable preferences use `workspace_preferences`; instructions are versioned separately. Models and billing read live registry, key-status, usage and credit data. Authentication and invitation delivery are still explicitly identified as unavailable rather than simulated.
...
- [x] **Help** page with searchable guides, FAQs, shortcuts and troubleshooting links.
